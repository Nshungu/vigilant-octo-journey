<template>
  <div class="container mx-auto px-4 py-8">
    <div class="flex justify-between items-center mb-8">
      ...
      <div v-if="wishlistStore.items.length > 0" class="flex space-x-4">
        <button
          @click="moveAllToCart"
          class="btn-primary"
        >
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
      ...
      >
        <div class="relative">
          ...
          <!-- In Stock Badge -->
          <div class="absolute top-4 left-4 bg-green-500 text-white px-2 py-1 text-xs font-semibold rounded">
            In Stock
          </div>
        </div>
    ...
    <!-- Wishlist Stats -->
    <div v-if="wishlistStore.items.length > 0" class="mt-12 bg-white rounded-lg shadow-md p-6">
      <h2 class="text-xl font-semibold mb-4">Wishlist Summary</h2>
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div class="text-center">
          <div class="text-3xl font-bold text-kigali-blue mb-2">
        ...
        <div class="text-center">
          <div class="text-3xl font-bold text-kigali-yellow mb-2">
            {{ inCartCount }}
          </div>
          <p class="text-gray-600">Already in Cart</p>
        </div>
      </div>
    </div>
  </div>
...
const wishlistTotal = computed(() => {
  return wishlistStore.items.reduce((total, item) => total + item.price, 0)
})

const inCartCount = computed(() => {
  return wishlistStore.items.filter(item => cartStore.isInCart(item.id)).length
})
